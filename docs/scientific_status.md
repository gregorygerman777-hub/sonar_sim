...
| Water-column propagation | External Fortran BELLHOP execution; generated linear SSP environment; parsed ASCII rays and complex-amplitude arrivals; direct-delay analytic check | GPL solver installed separately; range-depth only; outputs are visualized but not yet coupled into two-way FSS target scattering |
