...
| Multipath | Surface and seabed eigenray paths (flat-boundary image method), independent component switches per boundary, projected mirror bearing, roll, coherent roughness sweep, both real-space legs occlusion-tested; seabed uses a Rayleigh two-fluid reflection coefficient with a critical grazing angle, see [eigenray_multipath_20260913](eigenray_multipath_20260913.md) | Single bounce per boundary only (no surface-plus-bottom combination paths); no sound-speed-profile refraction; diffuse rough-surface energy, pressure phase and full concavity reverberation incomplete |
...
| Navigation | Planar IMU integration, sonar local maxima, scan descriptors, mutual-nearest ICP, robust C++ SE(2) pose graph, loop closure, feature map and marginal covariance | Seeded synthetic closed loop; first pose fixed; no 6-DoF, estimated IMU bias, synchronization, pressure sensor, visual odometry or real-data calibration |
| Water-column propagation | External Fortran BELLHOP execution; generated linear SSP environment; parsed ASCII rays and complex-amplitude arrivals; reciprocal two-way path pairing; FSS range-bin deposition; direct-delay and hand-derived pairing checks | GPL solver installed separately; range-depth only; one target depth assigned after elevation collapse; incoherent intensity sum; no per-patch environment or broadband phase interference |
