...
## 26. SIFT keypoints were a quarter pixel off (front end error, fixed)

* **Found by measurement** (it was noted on 2026-09-24 and first left alone as library behaviour): OpenCV's SIFT
  reports each keypoint 0.25 px right of and below its position. It detects on the image upsampled by 2 and halves the
  coordinates without the half pixel shift of the pixel centre convention. On Gaussian blobs at known sub pixel
  centres the offset is +0.255 px in x and y (spread 0.02 px). K is in the pixel centre convention, so every SIFT run
  had its principal point effectively a quarter pixel off. COLMAP's own SIFT is not affected (entry 17 handles its
  convention).
* **Fix:** `features.Extractor` subtracts `SIFT_KEYPOINT_OFFSET = 0.25` from SIFT keypoints; ORB is unchanged.
  `tests/test_features.py` **fails** on the old code (0.255 px) and passes now.
* **Checked on ORB too:** OpenCV scales ORB's pyramid keypoints by the level's scale about the pixel corner, not the
  pixel centre. Correcting that on the rendered pool scene changed the ATE from 0.522 to 0.545 cm and the map error
  from 1.102 to 1.097 cm, the size of the change any small code change causes; no error could be shown, and ORB is left
  as ORB-SLAM uses it.
* **Result:** all 22 SIFT runs were repeated and every run rescored (`run_all.sh --only-eval`); ORB and COLMAP runs are
  unaffected (the audit compares their code with the SIFT line left out). Before and after, generated by
  `tools/before_after.py 051b8a7 --committed-only --frontend=sift`:

| Run | Before | After |
|---|---|---|
| synthetic_pool_caustics_sift | 240/240, 0.58 cm (0.06 %) | 240/240, 0.62 cm (0.06 %) |
| synthetic_pool_sift | 240/240, 0.23 cm (0.02 %) | 240/240, 0.21 cm (0.02 %) |
| tum_freiburg1_xyz_sift | 798/798, 2.35 cm (0.29 %) | 798/798, 2.39 cm (0.30 %) |
| tum_freiburg3_long_office_household_sift | 2585/2585, 5.28 cm (0.24 %) | 2585/2585, 7.33 cm (0.33 %) |
| aqualoc_harbor_07_sift | 758/2261 (4 maps), 0.68 cm (0.08 %); all maps 1690 frames, 0.54 cm | 758/2261 (4 maps), 0.80 cm (0.09 %); all maps 1690 frames, 0.62 cm |
| kitti_00_sift | 4541/4541, 107.14 m (2.88 %) | 4541/4541, 107.26 m (2.88 %) |
| kitti_01_sift | 478/1101 (4 maps), 7.07 m (0.63 %); all maps 1066 frames, 5.73 m | 478/1101 (4 maps), 7.37 m (0.66 %); all maps 1066 frames, 5.93 m |
| kitti_02_sift | 4661/4661, 121.51 m (2.40 %) | 4661/4661, 122.20 m (2.41 %) |
| kitti_03_sift | 801/801, 1.51 m (0.27 %) | 801/801, 1.49 m (0.26 %) |
| kitti_04_sift | 271/271, 1.20 m (0.30 %) | 271/271, 1.21 m (0.31 %) |
| kitti_05_sift | 2761/2761, 51.70 m (2.34 %) | 2761/2761, 51.53 m (2.34 %) |
| kitti_06_sift | 1101/1101, 60.51 m (4.91 %) | 1101/1101, 60.52 m (4.91 %) |
| kitti_07_sift | 1101/1101, 16.61 m (2.39 %) | 1101/1101, 16.56 m (2.38 %) |
| kitti_08_sift | 4071/4071, 114.76 m (3.56 %) | 4071/4071, 115.48 m (3.58 %) |
| kitti_09_sift | 1591/1591, 80.76 m (4.74 %) | 1591/1591, 81.11 m (4.76 %) |
| kitti_10_sift | 1201/1201, 15.84 m (1.72 %) | 1201/1201, 16.00 m (1.74 %) |
| pool_raw_sift | 11/117 (6 maps) | 11/117 (6 maps) |
| pool_width_scaled_sift | 11/117 (5 maps) | 11/117 (5 maps) |

The quarter pixel moves the synthetic scene with exact ground truth the right way (0.23 to 0.21 cm) and everything
else by amounts of the size any small change causes, with one exception: TUM fr3 with SIFT went from 5.28 to 7.33 cm.
That 22 m loop has no loop closure, and its accumulated drift depends on small perturbations; the two numbers show how
large that sensitivity is, not a better or worse front end. No conclusion changes: TUM stays 1.7 to 3.9 times less
accurate than COLMAP, and SIFT still tracks every frame of 10 of the 11 KITTI sequences. The SIFT numbers in the table
after entry 19 are superseded by this one.
